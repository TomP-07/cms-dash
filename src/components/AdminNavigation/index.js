...
class AdminNavigation extends Component {
      ...
      <nav className="h-full overflow-y-scroll scrollbar scrollbar-track-transparent scrollbar-thumb-scrollBar hover:scrollbar-thumb-scrollBar">
        <ul id="AdminNavList" className={`${isExpanded ? "mr-5" : "mr-1"}`}>
          ...
          {this.props.data.entries.map((item, index) => {
            return this.renderNavigationEntry(
              { id: index, ...item },
              isExpanded
            );
          })}
  ...
  renderNavigationEntry(entry, isExpanded) {
    ...
    const isOpen =
      isExpanded &&
      isMenu &&
      (this.state.actMenu == ID ||
        (this.state.actItemMenu == ID && this.state.expItemMenu));

    let itemClass = classNames({
      menu: isMenu,
      "menu-open": isOpen,
      selected:
        this.state.actItem == ID ||
        this.state.actItemMenu == ID ||
        this.state.actMenu == ID,
    });
    return (
      <li
    ...
    return (
      <Link href={href}>
        ...
        >
          ...
          <div
            className={`${
              this.state.expanded ? "mr-3" : "mr-0"
            } icon flex w-min items-center justify-center`}
          >
          ...
          {isMenu && (
            <m.div
              ...
              className={`${
                this.state.expanded ? "" : "hidden"
              } menu-icon absolute right-3 top-1/2`}
            >
              <this.ChevronRightSvg
                width={30}
                height={30}
                className="h-6 w-6 fill-[#3a3541de]"
              />
            </m.div>
          )}
        </a>
